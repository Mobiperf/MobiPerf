...
public class MeasurementResult {
	...
	private void getHttpResult(StringBuilderPrinter printer,
		...
		printer.println("\nDownloaded " + (headerLen + bodyLen) + " bytes in "
				+ time + " ms");
	...
	private void getTracerouteResult(StringBuilderPrinter printer,
		...
		for (int i = 0; i < hops; i++) {
			...
			printer.println(hopInfo + "\t\t" + String.format("%.1f", time)
					+ " ms");
		}
	}
	...
	/**
	 * Removes the quotes surrounding the string. If the string is less than 2
	 * in length, we returns null
	 */
