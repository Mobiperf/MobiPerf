...
public class PingTask extends MeasurementTask {
	...
	/**
	 * Encode ping specific parameters, along with common parameters inherited
	 * from MeasurmentDesc
	 * 
	 * @author wenjiezeng@google.com (Steve Zeng)
	 * 
	...
	public static class PingDesc extends MeasurementDesc {
		...
		public PingDesc(String key, Date startTime, Date endTime,
			...
			if (this.target == null || this.target.length() == 0) {
				throw new InvalidParameterException(
						"PingTask cannot be created due "
		...
		protected void initalizeParams(Map<String, String> params) {
			...
			try {
				...
				throw new InvalidParameterException(
						"PingTask cannot be created due to invalid params");
			}
		}
	...
	public PingTask(MeasurementDesc desc, Context parent) {
		super(new PingDesc(desc.key, desc.startTime, desc.endTime,
	...
	/*
	 * We will use three methods to ping the requested resource in the order of
	 * PING_COMMAND, JAVA_ICMP_PING, and HTTP_PING. If all fails, then we
	 * declare the resource unreachable
	 */
	@Override
	public MeasurementResult call() throws MeasurementError {
		...
		try {
			...
			/*
			 * Prevents the phone from going to low-power mode where WiFi turns
			 * off
			 */
	...
	private MeasurementResult constructResult(ArrayList<Double> rrtVals,
			double packetLoss, int packetsSent) {
		...
		MeasurementResult result = new MeasurementResult(
				...
				phoneUtils.getDeviceProperty(), PingTask.TYPE,
	...
	/*
	 * Compute the average of the filtered rtts. The first several ping results
	 * are usually extremely large as the device needs to activate the wireless
	 * interface and resolve domain names. Such distorted measurements are
	 * filtered out
	 */
	...
	private MeasurementResult executePingCmdTask() throws MeasurementError {
		...
		try {
			String command = Util.constructCommand(pingTask.pingExe, "-i",
					Config.DEFAULT_INTERVAL_BETWEEN_ICMP_PACKET_SEC, "-s",
					pingTask.packetSizeByte, "-w", pingTask.pingTimeoutSec,
					"-c", Config.PING_COUNT_PER_MEASUREMENT, targetIp);
			...
			while ((line = br.readLine()) != null) {
				...
				// output
				int[] packetLossInfo = Util
						.extractPacketLossInfoFromPingOutput(line);
	...
	private MeasurementResult executeJavaPingTask() throws MeasurementError {
		...
		try {
			...
			for (int i = 0; i < Config.PING_COUNT_PER_MEASUREMENT; i++) {
				...
				boolean status = InetAddress.getByName(targetIp).isReachable(
						timeOut);
	...
	/**
	 * Use the HTTP Head method to emulate ping. The measurement from this
	 * method can be substantially (2x) greater than the first two methods and
	 * inaccurate. This is because, depending on the implementing of the
	 * destination web server, either a quick HTTP response is replied or some
	...
	private MeasurementResult executeHttpPingTask() throws MeasurementError {
		...
		try {
			...
			HttpClient client = AndroidHttpClient.newInstance(Util
					.prepareUserAgent(this.parent));
			...
			int timeOut = (int) (1000 * (double) pingTask.pingTimeoutSec / Config.PING_COUNT_PER_MEASUREMENT);
			HttpConnectionParams.setConnectionTimeout(headMethod.getParams(),
					timeOut);
