...
public class HttpTask extends MeasurementTask {
	...
	/*
	 * TODO(Wenjie): Depending on state machine configuration of cell tower's
	 * radio, the size to find the 'real' bandwidth of the phone may be network
	 * dependent.
	 */
	...
	public HttpTask(MeasurementDesc desc, Context parent) {
		super(new HttpDesc(desc.key, desc.startTime, desc.endTime,
	...
	/**
	 * Runs the HTTP measurement task. Will acquire power lock to ensure wifi is
	 * not turned off
	 */
	@Override
	public MeasurementResult call() throws MeasurementError {
		...
		try {
			...
			httpClient = AndroidHttpClient.newInstance(Util
			...
			if (task.headers != null && task.headers.trim().length() > 0) {
				for (String headerLine : task.headers.split("\r\n")) {
					...
					if (tokens.length == 2) {
						...
						throw new MeasurementError("Incorrect header line: "
								+ headerLine);
					}
				}
			}
			...
			/*
			 * TODO(Wenjie): HttpClient does not automatically handle the
			 * following codes 301 Moved Permanently.
			 * HttpStatus.SC_MOVED_PERMANENTLY 302 Moved Temporarily.
			 * HttpStatus.SC_MOVED_TEMPORARILY 303 See Other.
			 * HttpStatus.SC_SEE_OTHER 307 Temporary Redirect.
			...
			/*
			 * For HttpClient to work properly, we still want to consume the
			 * entire response even if the status code is not 200
			 */
			...
			if (responseEntity != null) {
				...
				while ((readLen = inputStream.read(readBuffer)) > 0
					...
					if (body.remaining() > 0) {
						int putLen = body.remaining() < readLen ? body
								.remaining() : readLen;
			...
			if (responseHeaders != null) {
				...
				for (Header hdr : responseHeaders) {
					/*
					 * TODO(Wenjie): There can be preceding and trailing white
					 * spaces in each header field. I cannot find internal
					 * methods that return the number of bytes in a header. The
					 * solution here assumes the encoding is one byte per
					 * character.
					 */
			...
			MeasurementResult result = new MeasurementResult(
					...
					phoneUtils.getDeviceProperty(), HttpTask.TYPE,
			...
			if (success) {
				...
				result.addResult("body",
						Base64.encodeToString(body.array(), Base64.DEFAULT));
			}
		...
		throw new MeasurementError(
				"Cannot get result from HTTP measurement because " + errorMsg);
	}
