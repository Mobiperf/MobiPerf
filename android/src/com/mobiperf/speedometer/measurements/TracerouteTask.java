...
public class TracerouteTask extends MeasurementTask {
	...
	public static class TracerouteDesc extends MeasurementDesc {
		...
		public TracerouteDesc(String key, Date startTime, Date endTime,
			...
			if (target == null || target.length() == 0) {
				throw new InvalidParameterException(
						"Target of traceroute cannot be null");
			}
		}
		...
		protected void initalizeParams(Map<String, String> params) {
			...
			try {
				...
				if ((val = params.get("max_hop_count")) != null
						&& val.length() > 0 && Integer.parseInt(val) > 0) {
	...
	public TracerouteTask(MeasurementDesc desc, Context parent) {
		super(new TracerouteDesc(desc.key, desc.startTime, desc.endTime,
	...
	public MeasurementResult call() throws MeasurementError {
		...
		try {
			...
			throw new MeasurementError("target " + target
					+ " cannot be resolved");
		}
		...
		while (maxHopCount-- >= 0 && !stopRequested) {
			/*
			 * Current traceroute implementation sends out three ICMP probes per
			 * TTL. One ping every 0.2s is the lower bound before some platforms
			 * requires root to run ping. We ping once every time to get a rough
			 * rtt as we cannot get the exact rtt from the output of the ping
			 * command with ttl being set
			 */
			String command = Util.constructCommand(task.pingExe, "-n", "-t",
					ttl, "-s", task.packetSizeByte, "-c 1 ", target);
			try {
				double rtt = 0;
				long t1;
				...
				for (int i = 0; i < task.pingsPerHop; i++) {
					...
					/*
					 * Process each line of the ping output and extracts the
					 * intermediate hops into hostAtThisDistance
					 */
				...
				for (String ip : hostsAtThisDistance) {
					...
					if (ip.compareTo(hostIp) == 0) {
						Logger.i(ttl + ": " + hostIp);
						Logger.i(" Finished! " + target + " reached in " + ttl
								+ " hops");
						...
						for (int i = 0; i < hopHosts.size(); i++) {
							...
							for (String host : hopInfo.hosts) {
								result.addResult("hop_" + i + "_addr_"
										+ hostIdx++, host);
							}
				...
				progressStr.delete(progressStr.length() - 3,
						progressStr.length());
		...
		throw new MeasurementError("cannot perform traceroute to "
				+ task.target);
	}
	...
	private void processPingOutput(BufferedReader br,
			HashSet<String> hostsAtThisDistance, String hostIp)
			throws IOException {
		String line = null;
	...
	/*
	 * TODO(Wenjie): The current search for valid IPs assumes the IP string is
	 * not a proper substring of the space-separated tokens. For more robust
	 * searching in case different outputs from ping due to its different
	 * versions, we need to refine the search by testing weather any substring
	 * of the tokens contains a valid IP
	 */
