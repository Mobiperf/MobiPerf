...
public class DnsLookupTask extends MeasurementTask {
		...
		public DnsLookupDesc(String key, Date startTime, Date endTime,
			...
			if (this.target == null || this.target.length() == 0) {
				throw new InvalidParameterException(
						"LookupDnsTask cannot be created due "
	...
	public MeasurementTask clone() {
		...
		DnsLookupDesc newDesc = new DnsLookupDesc(desc.key, desc.startTime,
	...
	public MeasurementResult call() throws MeasurementError {
		...
		for (int i = 0; i < Config.DEFAULT_DNS_COUNT_PER_MEASUREMENT; i++) {
			try {
				...
				this.progress = 100 * i
						/ Config.DEFAULT_DNS_COUNT_PER_MEASUREMENT;
		...
		if (resultInet != null) {
			...
			MeasurementResult result = new MeasurementResult(
					...
					phoneUtils.getDeviceProperty(), DnsLookupTask.TYPE,
