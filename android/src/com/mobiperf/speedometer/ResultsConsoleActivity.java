...
public class ResultsConsoleActivity extends Activity {
			...
			public void onReceive(Context context, Intent intent) {
				if (intent.getAction().equals(
					...
					int progress = intent.getIntExtra(
							UpdateIntent.PROGRESS_PAYLOAD,
							Config.INVALID_PROGRESS);
					int priority = intent.getIntExtra(
							UpdateIntent.TASK_PRIORITY_PAYLOAD,
							MeasurementTask.INVALID_PRIORITY);
					// Show user results if we there is currently a user
					// measurement running
	...
	/**
	 * Change the underlying adapter for the ListView depending on the old and
	 * new value of showUserResults. userResults is the ArraytAdapter that
	 * stores user measurement results and systemResults is the ArrayAdapter
	 * that stores system results. Depending on which one is the underlying
	 * ArrayAdapter of consoleView, we see either user or system results.
	 * 
	 * @param oldShowUserResults
	 *            the old value of showUserResults
	 * @param newShowUserResults
	 *            the new value of showUserResults
	 */
	private void switchBewteenResults(boolean oldShowUserResults,
			boolean newShowUserResults) {
		getConsoleContentFromScheduler();
	...
	private void upgradeProgress(int progress, int max) {
		...
		if (progress >= 0 && progress <= max) {
			...
			/*
			 * UserMeasurementTask broadcast a progress greater than max to
			 * indicate the termination of the measurement
			 */
